...
`eval metrics` 计算 Precision、Recall、mAP@0.5、mAP@0.5:0.95，并支持 `--class` 只评估指定类别；未选类别的 GT 和预测都会被忽略。小目标过滤可使用 `--min-width`、`--min-height`、`--min-area`、`--min-size-logic`，或按像素使用 `--min-pixels`。
