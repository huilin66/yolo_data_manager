...
mgr.vis_crop(out="crops", filter_no_attrs=True, att_seperate=True)  # 从 crop 结果复制到 crop_att/attribute/value
...
`ensure_class_presence=True`（默认）会按图像中的类别分布进行启发式分配，尽量让每个有标注的类别出现在每个非空 split 中；当类别样本不足时，覆盖优先级为 train > test > val，`test=0` 时为 train > val。类别图片数量不足、include list 固定了分配，或 split 容量不足时无法完全保证；传 `False` 可关闭该策略。
